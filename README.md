...
そこで、私は以下の二つのモデルについて光度曲線に対する各パラメータの影響を検証しています。一つ目は、前駆星が連星系をなし、星周物質（CSM）が周期的な密度構造をもつ場合で、CSMに衝撃波が次々に衝突する**CSM相互作用モデル**です。二つ目は、爆発後にも連星系を維持している場合で、離心率の大きな伴星から中心天体（magnetar）に周期的な質量降着が起こる**magnetar駆動モデル**です。
