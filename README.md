...
近年、**周期的光度変動**を示す超新星爆発が観測されるようになりましたが、その理論モデルはいまだに**明らかではありません**。

そこで、私は以下の二つのモデルについて光度曲線に対する各パラメータの影響を検証しています。

一つ目は、前駆星が連星系をなし、星周物質（CSM）が周期的な密度構造をもつ場合で、CSMに衝撃波が次々に衝突する**CSM相互作用モデル**です。

二つ目は、爆発後にも連星系を維持している場合で、離心率の大きな伴星から中心天体（magnetar）に周期的な質量降着が起こる**magnetar駆動モデル**です。

爆発のエネルギーや噴出物の質量、変動周期などのパラメータを変化させることで、**初期条件の変動がそのまま現れたり、平滑化して現れなかったりする**ことがわかっています。

将来的には、より広範なパラメータサーベイを行い、**光度曲線の形状や周期から、超新星の様子や爆発前後の連星周期を推定すること**を目指しています。
...
1. **CSM付き初期プロファイル**: 親星の外側にCSMの密度分布を付加した初期モデル（`simple1d_plus_csm.short` など）を使用。プロファイルには密度、温度、速度、電子分率(Ye) などが含まれます。`setup_simple1d_snic_for_snec_elem30.f` と `setup_snec_elem30_snic.f` をコンパイルすることで作成することができます。必要に応じて `profiles` ディレクトリ内の `simple1d_plus_csm_*.f` を編集してください。
...
### 各データの可視化
各データは、`analytics` ディレクトリにある Python スクリプトで可視化することができます。例えば、`analytics/L_obs_plot.py` はボロメトリック光度曲線をプロットします。 `analytics/r_rho.py` は指定した時間における密度と半径の関係をプロットします。
