...
!----------- update the temperature, pressure and internal energy -------------
  ...
  if(time.ge.time_Ni) then
   ...
   ! -------- Ouchi wrote this.  -------- 
   !calculate heating term due to magnetar
   if(time.ge. 0) then ! time in unit of sec. ! magnetar injection starts from the beginning.
   !   time_magnetar = time_magnetar + Ni_period
      call magnetar_heat
   endif
! -------- Ouchi wrote this.  -------- 
