subroutine analysis
  ...
  real*8 :: Lsd, tsd_sec ! Ouchi wrote this.
  real*8 :: T_eff_for_BC
...
!------------------- Calculate the observed luminosity ------------------------
  ...
  tsd_sec = tsd*86400d0 ! Spin-down timescale (sec) ! Ouchi wrote this.
  ...
  if(photosphere_fell_on_the_center.eq.0) then
     lum_observed = lum_photo + sum(Ni_energy_rate* &
          ...
          sum(magnetar_heating(index_photo:imax)*delta_mass(index_photo:imax)) ! Ouchi wrote this. magnetar heating.
  else
     lum_observed = lum(1) + &
          sum(Ni_energy_rate*Ni_deposit_function(1:imax)*delta_mass(1:imax)) + &
          Lsd_ini*(1d0 + time/tsd_sec)**(-2)  ! Ouchi wrote this. magnetar heating.
