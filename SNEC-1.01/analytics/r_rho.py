...
rho_data = load_xg_by_time_prefix(
...
if time_prefix not in radius_data or time_prefix not in rho_data:
    raise ValueError(f"time prefix {time_prefix} not found")

df_r = radius_data[time_prefix]
df_rho = rho_data[time_prefix]
...
# 黒色の線でプロット
ax.plot(df["radius"], df["density"], lw=1.5, color='black')
...
ax.set_title(f"Radius–Density relation (Time = {time_prefix})", fontsize=20)
